...
fs.readFile('lista.txt', 'utf8', (err, data) => {
  if (err) {
    console.error('Erro ao ler o arquivo lista.txt:', err);
    return;
  }
    ...
    linhas.forEach(linha => {
      if (linha.trim() !== '') {
        ...
        if (partes.length >= 2) {
          ...
          const nomeCompleto = partes[1].trim().replace(/\r/g, '').replace(/\n/g, '');
          ...
          registros.push(`${cpf}:${email}:${nomeCompleto}:${telefone}:${endereco}`);
        }
      }
    });
